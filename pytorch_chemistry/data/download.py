...
_dataset_root = os.environ.get('E2EDD_DATASET_ROOT',
                               DATAPATH)
...
def set_dataset_root(path):
    ...
    environment variable ``E2EDD_DATASET_ROOT``. The other is by using this
    function. If both are specified, one specified via this function is used.
    The default dataset root is ``$HOME/.e2edd/dataset``.
    Args:
